			...
			if active {
				...
				if err != nil {
					...
					if !task.Interval.Valid {
						if Error != nil {
							dev.LogError(Error, "Task "+strconv.FormatInt(task.ID, 10)+" failed: "+Error.Error())
						...
						if Error != nil {
							dev.LogError(Error, "Reoccuring Task "+strconv.FormatInt(task.ID, 10)+" failed: "+Error.Error())
						} else {
