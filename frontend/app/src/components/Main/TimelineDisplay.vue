        ...
        <v-timeline v-else dense>
            <v-timeline-item v-for="(action) in CurrentTicket.Actions" :key="action.ID">
              <v-card class="elevation-2">
                ...
                <v-card-subtitle>{{action.Title}} <i>by</i>  <span v-if="action.IssuedBy.Valid">{{action.IssuedBy.Issuer.Firstname}} {{action.IssuedBy.Issuer.Lastname}} ({{action.IssuedBy.Issuer.Username}})</span>
                <span v-else><b>System</b></span></v-card-subtitle>
